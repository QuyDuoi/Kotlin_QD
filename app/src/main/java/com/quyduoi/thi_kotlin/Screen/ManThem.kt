...
@Composable
fun ManThem(navController: NavHostController, viewModel: SanPhamViewModel) {
    Box(modifier = Modifier.fillMaxSize(),
        Alignment.Center) {
        var PH41939_mssv by remember { mutableStateOf("") }
        var PH41939_diemTB by remember { mutableStateOf("")}
        var PH41939_tensv by remember { mutableStateOf("") }
        ...
        if (showToastAndNavigate) {
            LaunchedEffect(Unit) {
                ...
                navController.navigate(Screens.TrangChu.screen) {
                    popUpTo(Screens.ManThem.screen) { inclusive = true }
                }
        ...
        Card(
            ...
            Column(
                ...
                Row(
                    ...
                    Button(
                        onClick = {
                            val priceDouble = PH41939_diemTB.toDoubleOrNull() ?: 0.0
                            if (PH41939_mssv.trim().isEmpty() || PH41939_diemTB.trim().isEmpty() || PH41939_tensv.trim().isEmpty()) {
                                ...
                                val result = viewModel.them(newSinhVien)
                                if (result != null) {
